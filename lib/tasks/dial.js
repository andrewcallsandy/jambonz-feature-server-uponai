...
class TaskDial extends Task {
  ...
  async _attemptCalls(cs) {
    ...
    const fromUri = parsedFrom?.uri ? parseUri(parsedFrom.uri) : undefined;
    const opts = {
      headers: this.headers,
      proxy: `sip:${sbcAddress}`,
      callingNumber: this.callerId || fromUri?.user || req.callingNumber,
  ...
  async _onReinvite(req, res) {
    try {
      this.logger.debug({
        cseq: req.get('CSeq'),
        isOnHoldEnabled: this.isOnHoldEnabled,
        isOnhold: isOnhold(req.body),
        hasEp: !!this.ep,
        hasEpOther: !!this.epOther,
        mediaPath: this._mediaPath,
        offer: req.body
      }, 'REINVITE_TRACE dial._onReinvite ENTER (B leg)');
      let isHandled = false;
 ...
 media already released, reconnect media server`);
      ...
      if (!isHandled) {
        this.logger.debug('REINVITE_TRACE dial._onReinvite BRANCH passthrough -> cs.handleReinviteAfterMediaReleased');
        await this.cs.handleReinviteAfterMediaReleased(req, res);
      }
      this.logger.debug({isHandled}, 'REINVITE_TRACE dial._onReinvite EXIT (B leg)');
    } catch (err) {
      this.logger.error({
        err,
        stack: err?.stack,
        ...
        message: err?.message,
        cseq: req.get('CSeq')
      }, 'REINVITE_TRACE dial._onReinvite ERROR (B leg)');
  ...
  async _releaseMedia(cs, sd, releaseEntirely = false) {
    ...
    /* The callee leg is released first, so cs.ep stays truthy but doomed for the whole of this
       method - not just for cs.releaseMediaToSBC.  Publish the release across both legs so an
       in-dialog re-INVITE arriving anywhere in here waits for it to finish instead of being
       pointed at an endpoint we are about to destroy. */
    return cs.trackMediaRelease(async() => {
      try {
  ...
  async reAnchorMedia(cs, sd) {
    ...
    /* Re-anchoring replaces cs.ep and re-INVITEs both legs, so it is the same class of window as
       a media release: publish it so a concurrent in-dialog re-INVITE waits for settled state
       rather than racing a half-built media path. */
    return cs.trackMediaRelease(() => this._reAnchorMedia(cs, sd));
  }

  async _reAnchorMedia(cs, sd) {
    this.logger.info('Dial:reAnchorMedia - re-anchoring media to freeswitch');

    /* remember the original media path in case we need to revert on partial failure below */
    const originalMediaPath = this._mediaPath;
    const releaseEntirely = originalMediaPath === MediaPath.NoMedia;

    /*
     * Re-anchor legs one at a time (not in parallel) so that if one leg's re-INVITE is
     * rejected by the far end, we can reliably revert the other leg (which may have already
     * succeeded) back to direct media instead of leaving the call in an inconsistent state
     * where one leg is routed through freeswitch and the other still expects direct RTP.
     */
    try {
      await sd.reAnchorMedia(this._mediaPath);
    } catch (err) {
      this.logger.info(err, 'Dial:reAnchorMedia - failed to re-anchor callee leg, aborting');
      throw err;
    }

    try {
      await cs.reAnchorMedia(this._mediaPath);
    } catch (err) {
      this.logger.info(err,
        'Dial:reAnchorMedia - failed to re-anchor caller leg, reverting callee leg to avoid inconsistent media');
      if (sd.ep && cs.dlg?.connected) {
        try {
          await sd.releaseMediaToSBC(cs.dlg.remote.sdp, sd.ep.local.sdp, releaseEntirely);
        } catch (revertErr) {
          this.logger.error(revertErr, 'Dial:reAnchorMedia - error reverting callee leg after partial failure');
        }
      }
      throw err;
    }
    ...
    this._mediaPath = MediaPath.FullMedia;
    this.logger.info(
      `Dial:reAnchorMedia - successfully re-anchored media to freeswitch, media path is now ${this._mediaPath}`);
  }
  ...
  async handleReinviteAfterMediaReleased(req, res) {
    this.logger.debug({
      ...
      hasDlg: !!this.dlg,
      mediaPath: this._mediaPath,
      isIncomingLegHold: this.isIncomingLegHold,
      offer: req.body
    }, 'REINVITE_TRACE dial.handleReinviteAfterMediaReleased ENTER');
    let isHandled = false;
    if (this.isOnHoldEnabled) {
      if (isOnhold(req.body)) {
        if (!this.epOther && !this.ep) {
          this.logger.debug('REINVITE_TRACE dial.handleReinviteAfterMediaReleased BRANCH hold-no-media');
        ...
        if (this.epOther && this.ep && this.isIncomingLegHold &&
          ...
          this.logger.debug('REINVITE_TRACE dial.handleReinviteAfterMediaReleased BRANCH unhold-release-media');
          // Offhold, time to release media
    ...
    if (!isHandled) {
      this.logger.debug('REINVITE_TRACE dial.handleReinviteAfterMediaReleased BRANCH passthrough BEFORE dlg.modify');
      const traceDlgModifyStarted = Date.now();
      const sdp = await this.dlg.modify(req.body);
      this.logger.debug({elapsedMs: Date.now() - traceDlgModifyStarted, answer: sdp},
        'REINVITE_TRACE dial.handleReinviteAfterMediaReleased AFTER dlg.modify');
      this.logger.info({sdp}, 'Dial:handleReinviteAfterMediaReleased - sent reinvite to B leg');
      this.logger.debug({answer: sdp}, 'REINVITE_TRACE dial.handleReinviteAfterMediaReleased SENDING 200');
      res.send(200, {body: sdp});
    }
    this.logger.debug('REINVITE_TRACE dial.handleReinviteAfterMediaReleased EXIT');
  }
