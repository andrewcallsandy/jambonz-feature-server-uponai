...
class SingleDialer extends Emitter {
  ...
  async reAnchorMedia(currentMediaRoute = MediaPath.PartialMedia) {
    ...
    try {
      ...
      this.logger.info(err, 'SingleDialer:reAnchorMedia - error re-anchoring, cleaning up endpoint');
      try {
        await this.ep.destroy();
      } catch (destroyErr) {
        this.logger.info(destroyErr, 'SingleDialer:reAnchorMedia - error destroying endpoint during cleanup');
      }
      this.ep = null;
      throw err;
    }
