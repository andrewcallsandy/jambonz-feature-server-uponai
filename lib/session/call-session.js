...
/* How long an in-dialog re-INVITE will wait for one of our own media-release transactions to
   finish before giving up.  Measured release windows are 75-153ms (2026-08-19/20), so 500ms is
   3-6x the observed worst case while still leaving ~860ms of the ~1.36s upstream teardown budget
   for the dlg.modify that actually answers the re-INVITE. */
const MEDIA_RELEASE_WAIT_MS = 500;
...
class CallSession extends Emitter {
  ...
  constructor({logger, application, srf, tasks, callInfo, accountInfo, rootSpan, memberId, confName, confUuid}) {
    ...
    /* Set to a promise while an offer/answer transaction of ours is destroying or replacing
       this.ep.  this.ep stays truthy for the whole of that transaction, so an in-dialog
       re-INVITE arriving in the window would otherwise be handed to a doomed endpoint; instead
       it awaits this and is then dispatched against settled state. */
    this._mediaReleaseInFlight = null;
 ...
 */
  ...
  */
  async _lccListenStatus(opts) {
    const status = opts.listen_status || opts.stream_status;

    /* an ad-hoc listen started via _lccListen (updateCall opts.listen) takes priority */
    if (this.backgroundTaskManager.isTaskRunning('listen')) {
      return this.backgroundTaskManager.getTask('listen').updateListen(status, opts.silence);
    }
    ...
    listenTask.updateListen(status);
  }

  /**
   * perform live call control -- start or stop an ad-hoc audio fork (listen) on an
   * already-active call, without requiring a listen/stream verb to have been declared
   * up front. If the call is currently bridged via a Dial verb whose media has been
   * released back to the SBC, media is re-anchored through freeswitch on both legs
   * first so the resulting fork captures both parties.
   * @param {object} opts
   * @param {string} opts.action - 'start' or 'stop'
   * @param {string} [opts.url] - websocket url to stream audio to (required for 'start')
  */
  async _lccListen(opts = {}) {
    const {action, ...listenOpts} = opts;

    if (action === 'stop') {
      if (!this.backgroundTaskManager.isTaskRunning('listen')) {
        this.logger.info('CallSession:_lccListen - no ad-hoc listen is running, ignoring stop');
        return false;
      }
      this.backgroundTaskManager.stop('listen');
      return true;
    }

    if (action !== 'start') {
      this.logger.info(`CallSession:_lccListen - invalid action ${action}`);
      return false;
    }

    if (this.backgroundTaskManager.isTaskRunning('listen')) {
      this.logger.info('CallSession:_lccListen - an ad-hoc listen is already running, ignoring start');
      return false;
    }
    if (!listenOpts.url) {
      this.logger.info('CallSession:_lccListen - url is required to start listen');
      return false;
    }
    if (!this.dlg) {
      this.logger.info('CallSession:_lccListen - call is not yet answered, cannot start listen');
      return false;
    }

    const task = this.currentTask;
    if (task && task.name === TaskName.Dial && task.mediaPath !== MediaPath.FullMedia) {
      try {
        this.logger.info(
          `CallSession:_lccListen - re-anchoring media (currently ${task.mediaPath}) before starting listen`);
        await task.updateMediaPath(MediaPath.FullMedia);
      } catch (err) {
        this.logger.info(err, 'CallSession:_lccListen - error re-anchoring media before starting listen');
        return false;
      }
    }

    try {
      await this.backgroundTaskManager.newTask('listen', {verb: 'listen', ...listenOpts});
    } catch (err) {
      this.logger.info(err, 'CallSession:_lccListen - error starting listen task');
      return false;
    }
    return true;
  }
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  async updateCall(opts, callSid) {
    ...
    try {
      ...
      if (opts.listen) {
        return this._lccListen(opts.listen);
      }
  ...
  async _onCommand({msgid, command, call_sid, queueCommand, tool_call_id, data}) {
    ...
    switch (command) {
      ...
      case 'listen':
        this._lccListen(data)
          .catch((err) => {
            this.logger.info({err, data}, 'CallSession:_onCommand - error starting/stopping listen');
          });
        break;
  ...
  async _onReinvite(req, res) {
    try {
      this.logger.debug({
        ...
        callId: this.callId,
        cseq: req.get('CSeq'),
        currentTask: this.currentTask?.name,
        isOnHoldEnabled: this.currentTask?.isOnHoldEnabled,
        isSipRecCallSession: this.isSipRecCallSession,
        hasEndpoint: !!this.ep,
        epUuid: this.ep?.uuid,
        epConnected: this.ep?.connected,
        /* true if this re-INVITE arrived while one of our own media-release/re-anchor
           transactions was still in flight - the glare window that used to hang. */
        releasingMedia: this.mediaReleasePending,
        offer: req.body
      }, 'REINVITE_TRACE feature-server ENTER');

      /* Serialize against our own media release rather than racing it.  During a release this.ep
         is still truthy but doomed, and ep.modify() against it issues an ESL channel-variable
         query that freeswitch never answers once the channel is destroyed - the re-INVITE then
         stalls until the far end tears the call down.  Waiting costs ~60-150ms, after which
         this.ep is null and the media-released path answers with a real 200 OK. */
      if (this.mediaReleasePending) {
        ...
        this.logger.debug({
          ...
          epConnected: this.ep?.connected,
          awaitStarted
        }, 'REINVITE_TRACE feature-server BRANCH await-media-release');

        const released = await this.awaitMediaRelease();

        this.logger.debug({
          callSid: this.callSid,
          cseq: req.get('CSeq'),
          elapsedMs: Date.now() - awaitStarted,
          released,
          hasEndpoint: !!this.ep,
          epConnected: this.ep?.connected
        }, 'REINVITE_TRACE feature-server BRANCH await-media-release RETURNED');

        if (!released) {
          /* The release is wedged well beyond anything we have ever measured.  Falling through
             would drive ep.modify() into the same hang, so defer instead of failing: 491 completes
             the transaction and the far end retries a few seconds later, by which time the release
             has almost certainly settled.  A 500 here would be read as fatal and drop the call. */
          this.logger.debug({
            callSid: this.callSid,
            cseq: req.get('CSeq'),
            waitedMs: Date.now() - awaitStarted
          }, 'REINVITE_TRACE feature-server BRANCH await-media-release TIMED OUT sending 491');
          res.send(491);
          return;
        }
      }

      await this._dispatchReinvite(req, res);
    } catch (err) {
      /* An ESL channel-variable query that never came back tells us nothing about whether the
         offer is acceptable - the media renegotiation simply did not complete in time.  A 500 is
         read as fatal (2026-08-20 18:46:00.099: BroadWorks sent BYE 20ms after our 500), whereas
         491 completes the transaction and the far end retries successfully on this trunk
         (2026-08-20 18:46:24.095 -> retry at 18:46:27.132, call stayed up).  Map only these two
         codes; a genuine negotiation failure must still surface its own status. */
      const isEslStall = ['ESL_API_TIMEOUT', 'ESL_API_EMPTY_RESPONSE', 'ESL_API_QUEUE_SUSPECT']
        .includes(err?.code);
      const status = isEslStall ? 491 : (err.status || 500);
      this.logger.error({
        err,
        stack: err?.stack,
        status: err?.status,
        ...
        code: err?.code,
        isEslStall,
        sipStatusSent: status,
        callSid: this.callSid,
        epUuid: this.ep?.uuid
      }, 'REINVITE_TRACE feature-server ERROR');
  ...
  /**
   * Dispatch an in-dialog re-INVITE against current state.  Called only after any in-flight media
   * release has settled, so this.ep is read fresh here and never cached across an await.
   */
  async _dispatchReinvite(req, res) {
    const ep = this.ep;

    /* An endpoint whose fsmrf state is no longer CONNECTED cannot service ep.modify() - that ESL
       query is precisely what hangs - but it is functionally the same situation as media already
       having been released, so dispatch it that way and still answer 200 OK.  Compared against
       false explicitly so a future fsmrf without the `connected` getter degrades to "usable"
       rather than diverting every re-INVITE. */
    const epUsable = !!ep && ep.connected !== false;
    if (ep && !epUsable) {
      this.logger.debug({
        callSid: this.callSid,
        cseq: req.get('CSeq'),
        epUuid: ep.uuid,
        epState: ep.state
      }, 'REINVITE_TRACE feature-server BRANCH ep-not-connected -> handling as media-released');
    }

    if (epUsable) {
      if (this.isSipRecCallSession) {
        this.logger.debug({callSid: this.callSid}, 'REINVITE_TRACE feature-server BRANCH siprec');
        ...
        if (this.currentTask && this.currentTask.name === TaskName.Dial && this.currentTask.isOnHoldEnabled) {
          this.logger.debug({callSid: this.callSid},
            'REINVITE_TRACE feature-server BRANCH ep+dial-onhold -> task.handleReinviteAfterMediaReleased');
          ...
          this.logger.debug({callSid: this.callSid},
            'REINVITE_TRACE feature-server BRANCH ep+dial-onhold RETURNED');
        } else {
          this.logger.debug({callSid: this.callSid, epUuid: ep.uuid},
            'REINVITE_TRACE feature-server BRANCH ep-modify');
          this.logger.debug({callSid: this.callSid, epUuid: ep.uuid, offer: req.body},
            'REINVITE_TRACE feature-server BEFORE ep.modify');
          const epModifyStarted = Date.now();
          const newSdp = await ep.modify(req.body);
          this.logger.debug({
            callSid: this.callSid,
            epUuid: ep.uuid,
            elapsedMs: Date.now() - epModifyStarted,
            answer: newSdp
          }, 'REINVITE_TRACE feature-server AFTER ep.modify');
          this.logger.debug({callSid: this.callSid, answer: newSdp},
            'REINVITE_TRACE feature-server SENDING 200');
      ...
      this.logger.debug({callSid: this.callSid},
        'REINVITE_TRACE feature-server BRANCH no-ep+dial -> task.handleReinviteAfterMediaReleased');
      this.logger.info('handling reINVITE after media has been released');
      await this.currentTask.handleReinviteAfterMediaReleased(req, res);
      this.logger.debug({callSid: this.callSid}, 'REINVITE_TRACE feature-server BRANCH no-ep+dial RETURNED');
    }
    else {
      this.logger.debug({callSid: this.callSid, currentTask: this.currentTask?.name},
        'REINVITE_TRACE feature-server BRANCH no-ep-no-dial SENDING 488');
  ...
  /**
   * true while an offer/answer transaction of ours is destroying or replacing this.ep.
   */
  get mediaReleasePending() {
    return !!this._mediaReleaseInFlight;
  }

  /**
   * Run an operation that destroys or replaces this.ep, publishing it so that a concurrent
   * in-dialog re-INVITE can await its completion instead of racing it.
   *
   * The published promise NEVER rejects - a waiter only needs to know the release finished, not
   * whether it succeeded - so awaiting it cannot raise a spurious throw in _onReinvite nor leave
   * an unhandled rejection.  The operation's own result/rejection is passed through to its caller
   * unchanged.  Nested calls (Dial:_releaseMedia bracketing both legs, then releaseMediaToSBC
   * bracketing the caller leg) reuse the outermost promise so the window is never closed early.
   */
  async trackMediaRelease(fn) {
    if (this._mediaReleaseInFlight) return fn();

    let markDone;
    this._mediaReleaseInFlight = new Promise((resolve) => markDone = resolve);
    try {
      return await fn();
    } finally {
      this._mediaReleaseInFlight = null;
  ...
  /**
   * Wait for an in-flight media release, bounded.  Resolves true if it completed, false if the
   * bound elapsed first.
   */
  async awaitMediaRelease() {
    const inFlight = this._mediaReleaseInFlight;
    ...
    try {
      return await Promise.race([
        inFlight.then(() => true),
        new Promise((resolve) => timer = setTimeout(() => resolve(false), MEDIA_RELEASE_WAIT_MS))
      ]);
    } finally {
      clearTimeout(timer);
    }
  ...
  async releaseMediaToSBC(remoteSdp, releaseMediaEntirely) {
    ...
    return this.trackMediaRelease(async() => {
  ...
  async reAnchorMedia(currentMediaRoute = MediaPath.PartialMedia) {
    ...
    return this.trackMediaRelease(async() => {
      ...
      try {
        ...
        /* re-INVITE was rejected (or endpoint modify failed) - leave no dangling endpoint/state behind */
        this.logger.info(err, 'CallSession:reAnchorMedia - error re-anchoring, cleaning up endpoint');
        try {
          await this.ep.destroy();
        } catch (destroyErr) {
          this.logger.info(destroyErr, 'CallSession:reAnchorMedia - error destroying endpoint during cleanup');
        }
        this.ep = null;
        throw err;
      }
  ...
  async handleReinviteAfterMediaReleased(req, res) {
    this.logger.debug({
      ...
      cseq: req.get('CSeq'),
      hasDlg: !!this.dlg,
      dlgConnected: this.dlg?.connected,
      hasEndpoint: !!this.ep,
      offer: req.body
    }, 'REINVITE_TRACE feature-server cs.handleReinviteAfterMediaReleased ENTER');
    if (!this.dlg || !this.dlg.connected) {
      this.logger.info({
        hasDlg: !!this.dlg,
        dlgConnected: this.dlg?.connected,
      }, 'CallSession:handleReinviteAfterMediaReleased - dialog not connected, sending 491');
      this.logger.debug({callSid: this.callSid},
        'REINVITE_TRACE feature-server cs.handleReinviteAfterMediaReleased BRANCH not-connected SENDING 491');
      res.send(491);
      return;
    }
    if (this.ep) {
      this.logger.info(
        'CallSession:handleReinviteAfterMediaReleased - ep still set (race condition), proceeding anyway');
    }
    this.logger.debug({callSid: this.callSid},
      'REINVITE_TRACE feature-server cs.handleReinviteAfterMediaReleased BEFORE dlg.modify (A leg)');
    const traceDlgModifyStarted = Date.now();
    const sdp = await this.dlg.modify(req.body);
    this.logger.debug({callSid: this.callSid, elapsedMs: Date.now() - traceDlgModifyStarted, answer: sdp},
      'REINVITE_TRACE feature-server cs.handleReinviteAfterMediaReleased AFTER dlg.modify (A leg)');
    this.logger.debug({sdp}, 'CallSession:handleReinviteAfterMediaReleased - reinvite to A leg returned sdp');
    this.logger.debug({callSid: this.callSid, answer: sdp},
      'REINVITE_TRACE feature-server cs.handleReinviteAfterMediaReleased SENDING 200');
